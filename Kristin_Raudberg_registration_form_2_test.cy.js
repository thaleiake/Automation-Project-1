...
describe('Section 1: Functional tests', () => {
    ...
    cy.get('#username').type('KristinR')
    cy.get('#email').type('kristinraudberg@mail.com')
    cy.get('[data-cy="name"]').type('Kristin')
    cy.get('#lastName').type('Raudberg')
    cy.get('[data-testid="phoneNumberTestId"]').type('5566778899')

    // Type confirmation password which is different from first password
    cy.get('#password').type('Kuusepuu123')
    cy.get('#confirm').type('Kuusepuu1234')
    ...
    //Change the password and confirmation password to have the same values
    cy.get('#confirm').clear()
    cy.get('#confirm').type('Kuusepuu123')

    //Assert the submit button is activated
    cy.get('h2').contains('Password').click()
    cy.get('.submit_button').should('be.enabled')
    cy.get('.submit_button').click()

    //Assert that after submitting a successful message is shown
    ...
    cy.contains('I have a car').click()
    ...
    it('User is unable to submit form without last name', ()=>{
    cy.get('#username').type('KristinR')
    cy.get('#email').type('kristinraudberg@mail.com')
    cy.get('[data-cy="name"]').type('Kristin')
...
describe('Section 2: Visual tests', () => {
    ...
    // get element and check its parameter height, to less than 116 and greater than 100
    ...
    it('Check the second link to the Registration form 3', () => {
    cy.get('nav').children().should('have.length', 2)
    cy.get('nav').siblings('h1').should('have.text', 'Registration form number 2')
